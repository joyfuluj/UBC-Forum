...
### Registered Users:
    ...
    MINOR:
        - Allow Users to delete comments and posts (Tanner)
            Users can delete their own comments from anywhere on the site, and delete their posts from their account dashboard. Site Admins can delete all comments.
...
# Known Limitations
    Currently, there is no way to create admin accounts witout manually cahnging their privelege value to "2" in the database.
    Only Admin accounts can search for users, regular accounts cannot find other users.
    Joining a community doesn't update the user's homepage feed.
